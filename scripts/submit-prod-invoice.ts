...
const invoice = {
  ...
  "Invoice": [{
    ...
    "AccountingCustomerParty": [{
      "Party": [{
        "PartyIdentification": [{
          "ID": [{"_": "IG22570640070", "schemeID": "TIN"}]
        }, {
          "ID": [{"_": "901120125931", "schemeID": "NRIC"}]
        }],
        "PartyLegalEntity": [{
          "RegistrationName": [{"_": "ZAHID BIN ARAMAI"}]
        }],
        "PostalAddress": [{
...
async function main() {
  ...
  // IMPORTANT: Per farhan-syah/myinvois-client, the document hash in submission
  // should be SHA256 of the JSON string, HEX encoded (not base64!)
  ...
  // Per farhan-syah/myinvois-client: documentHash is SHA256 HEX (not base64!)
  const docHash = createHash("sha256").update(signedDocJson, "utf-8").digest("hex");

  console.log("\n=== DOCUMENT HASH ===");
  console.log("  Hash (hex):", docHash);
  
  console.log("Signed!");
  console.log("  JSON length:", signedDocJson.length);
  console.log("  Base64 length:", signedDocBase64.length);
  console.log("  Document hash (SHA256 of JSON):", docHash);
  ...
  console.log("Headers:");
  submitResp.headers.forEach((v, k) => console.log(" ", k + ":", v));
