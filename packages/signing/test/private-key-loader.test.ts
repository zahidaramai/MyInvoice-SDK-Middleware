...
import { PrivateKeyLoadError, KeyCertificateMismatchError } from "../src/errors.js";
...
describe("Private Key Loader", () => {
  ...
  describe("loadPrivateKeyFromFile", () => {
    ...
    it("throws PrivateKeyLoadError for non-existent file", () => {
      expect(() => loadPrivateKeyFromFile("/non/existent/path.pem")).toThrow(PrivateKeyLoadError);
    });
  });
  ...
  describe("loadPrivateKeyFromEnv", () => {
    ...
    it("throws PrivateKeyLoadError for missing env var", () => {
      ...
      expect(() => loadPrivateKeyFromEnv("MISSING_KEY")).toThrow(PrivateKeyLoadError);
    });
  });
  ...
  describe("parsePrivateKey", () => {
    ...
    it("throws PrivateKeyLoadError for encrypted key with wrong passphrase", () => {
      ...
      expect(() => parsePrivateKey(pem, "wrongpassword")).toThrow(PrivateKeyLoadError);
    });
    ...
    it("throws PrivateKeyLoadError for invalid PEM", () => {
      expect(() => parsePrivateKey("not a private key")).toThrow(PrivateKeyLoadError);
    });
  });
  ...
  describe("loadAndVerifyPrivateKey", () => {
    it("loads and verifies matching key", () => {
      ...
      const key = loadAndVerifyPrivateKey({ path: validKeyPath }, certPem, "Test Certificate");
    ...
    it("throws KeyCertificateMismatchError for mismatched key", () => {
      ...
      expect(() =>
        loadAndVerifyPrivateKey({ path: mismatchedKeyPath }, certPem, "Test Certificate")
