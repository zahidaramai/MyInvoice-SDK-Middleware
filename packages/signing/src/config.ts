...
export const CertificateSourceSchema = z
  ...
  .refine(
    ...
    {
      message: "Exactly one certificate source must be specified: path, envVar, or base64",
    }
  );
...
export const PrivateKeySourceSchema = z
  ...
  .refine(
    ...
    {
      message: "Exactly one private key source must be specified: path, envVar, or base64",
    }
  );
...
export const SigningConfigSchema = z
  ...
  .refine(
    ...
    {
      message:
        "When signing is enabled, either pkcs12 OR both certificate and privateKey must be configured",
    }
  )
  .refine(
    (data) => {
    ...
    {
      message:
        "Cannot specify both pkcs12 and separate certificate/privateKey - use one or the other",
    }
  );
...
export const SIGNING_ENV_VARS = {
  ...
  DEFAULT_VERSION: "SIGNING_DEFAULT_VERSION",
