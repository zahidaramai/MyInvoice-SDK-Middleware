...
import { submitDocuments, type SubmitDocumentsRequest } from "@myinvois/myinvois-client";
import {
  createSubmission,
...
import {
  enqueuePoll,
  enqueueImmediatePoll,
  calculatePollDelay,
  MIN_POLL_INTERVAL_MS,
} from "../../lib/pollQueue.js";
 ...
 */
function submissionToResponse(submission: SubmissionWithDocuments): SubmissionResultResponse {
...
export const submissionsRoutes: FastifyPluginAsync = async (fastify) => {
   ...
   */
  fastify.post<{ Body: CreateSubmissionBody }>("/v1/submissions", async (request, reply) => {
    ...
    const existingSubmission = await findRecentByPayloadHash(session.sessionId, payloadHash);
    ...
    if (!result.ok) {
      ...
      if (result.error.status === 0) {
        throw new AppError(normalized.httpStatus || 504, result.error.message, normalized.code, {
          correlationId: result.error.meta?.correlationId,
      ...
      if (result.error.status && result.error.status >= 500) {
        throw new AppError(result.error.status, result.error.message, normalized.code, {
          correlationId: result.error.meta?.correlationId,
          retryable: true,
        });
      }
  ...
  fastify.post<{ Params: { trackingId: string } }>(
    ...
    async (request, reply) => {
      ...
      request.log.info({ correlationId, trackingId, jobId }, "Manual poll enqueued");
